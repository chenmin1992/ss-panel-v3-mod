...
class Job
{
    ...
    public static function updateClashRules()
    {
        ...
        foreach ($rules as $index => $line) {
            ...
            if(!$started && strtoupper($rule) == '[RULE]') {
                $started = true;
                $data .= "DOMAIN,localhost,DIRECT\n";
                $data .= "IP-CIDR,127.0.0.0/8,DIRECT\n";
                $data .= "IP-CIDR,10.0.0.0/8,DIRECT\n";
                $data .= "IP-CIDR,172.16.0.0/12,DIRECT\n";
    ...
    public static function CheckJob()
    {
        ...
        $latest_content = file_get_contents("https://github.com/chenmin1992/ss-panel-v3-mod/raw/master/bootstrap.php");
